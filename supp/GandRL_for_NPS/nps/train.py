        ...
        for sp_idx in tqdm(range(0, len(dataset["sources"]), batch_size)):
            ...
            if signal == TrainSignal.SUPERVISED:
                ...
                if signal == TrainSignal.RL:
                    ...
                    minibatch_reward, pseudo_num = get_arm_loss(model,
                                                  inp_grids, out_grids,
                                                  envs,
                                                  tgt_start, tgt_end,
                                                  max_len, arm_sample, decay_factor, logits_factor)
                    recent_pseudo_num.append(pseudo_num)
                elif signal == 'ada_arm':
            ...
            if (batch_idx % log_frequency == log_frequency-1 and len(recent_losses) > 0) or \
               (len(dataset["sources"]) - sp_idx ) < batch_size:
                ...
                variances.extend(recent_variances)
                times.extend(recent_times)
                pseudos.extend(recent_pseudo_num)
                recent_losses = []
                recent_variances = []
                recent_times = []
                recent_pseudo_num = []
                train_info = {'losses': losses, 'variances': variances, 'times': times, 'pseudos': pseudos}
                # Dump the training losses
                with open(str(train_loss_path), "w") as train_loss_file:
                    json.dump(train_info, train_loss_file, indent=2)
                ...
                if signal == TrainSignal.BEAM_RL:
                    ...
                    with open(str(path_to_weight_dump), "wb") as weight_file:
                        ...
                        if use_cuda:
                            model.cuda()
            # if index % 2000 ==0:
            #     path_to_weight_dump = models_dir / ("weights_%d_%d.model" % (epoch_idx, index))
            #     with open(str(path_to_weight_dump), "wb") as weight_file:
            #         # Needs to be in cpu mode to dump, otherwise will be annoying to load
            #         if use_cuda:
            #             model.cpu()
            ...
            #         if use_cuda:
            #             model.cuda()
            #     out_path = str(result_dir / ("eval/epoch_%d_%d/val_.txt" % (epoch_idx, index)))
            ...
            #         logging.info("Epoch : %d Index: %d ValidationBest : %f." % (epoch_idx,index, val_acc))
